...
import pandas as pd
from .models import Feriado, Profile, Cliente, ClienteProspect, Servico, TipoServico, Meta, Tarefa, AcaoTarefa, Prospeccao, AcaoProspeccao, TarefaAgendada
from .forms import FeriadoForm, UserForm, ProfileForm, ServicoForm, MetaForm, CustomAuthenticationForm, TarefaForm, AcaoTarefaForm, ProspeccaoForm, AcaoProspeccaoForm, ClienteForm, ProspeccaoEditForm, ClienteProspectForm, TarefaAgendadaForm
...
# --- AUXILIARES ---
def get_dias_uteis_restantes(mes, ano, feriados_datas):
    """
    Retorna o número de dias úteis restantes a partir de 'hoje'.
    Inclui o dia de hoje se ele for útil.
    """
    hoje = date.today()
    if ano < hoje.year or (ano == hoje.year and mes < hoje.month):
        return 0
    
    # Se for mês futuro, conta o mês todo
    if ano > hoje.year or (ano == hoje.year and mes > hoje.month):
        start_day = 1
    else:
        start_day = hoje.day
    ...
    for day in range(start_day, last_day + 1):
        d = date(ano, mes, day)
        if d.weekday() < 5 and d not in feriados_datas:
            restantes += 1
    return restantes

# --- MIXINS ---
class GestaoRequiredMixin(UserPassesTestMixin):
    """Apenas COMERCIAL e ADMIN (acesso total - CRUD)"""
    def test_func(self):
        user = self.request.user
        return user.is_authenticated and (
            user.is_staff or user.profile.tem_acesso_gestao
        )
...
# Mixin para Gerente Operacional - Permite acesso mas só visualização
class GerenteOuSuperiorMixin(UserPassesTestMixin):
    """Permite acesso para Gerente Operacional e superiores (mas Gerente só visualiza)"""
    def test_func(self):
        user = self.request.user
        return user.is_authenticated and (
            user.is_staff or
            user.profile.tem_acesso_gestao or
            user.profile.is_gerente_operacional
        )

class ServicoMixin(UserPassesTestMixin):
    """Serviços: REPRESENTANTE, DIRETORIA (view), GERENTE (view), COMERCIAL, ADMIN"""
    def test_func(self):
        user = self.request.user
        return user.is_authenticated and (
            user.is_staff or
            user.profile.tem_acesso_gestao or
            user.profile.is_gerente_operacional or
            user.profile.is_diretoria or
            user.profile.is_representante
        )

class AgendaMixin(UserPassesTestMixin):
    """Agenda: REPRESENTANTE, DIRETORIA, COMERCIAL, ADMIN"""
    def test_func(self):
        user = self.request.user
        return user.is_authenticated and (
            user.is_staff or user.profile.pode_acessar_agenda
        )

class ProspeccaoMixin(UserPassesTestMixin):
    """Prospecção: REPRESENTANTE, DIRETORIA (view), COMERCIAL, ADMIN"""
    def test_func(self):
        user = self.request.user
        return user.is_authenticated and (
            user.is_staff or user.profile.pode_acessar_prospeccao
        )
...
class ClienteMixin(UserPassesTestMixin):
    ...
    def test_func(self):
        user = self.request.user
        return user.is_authenticated and (
            user.is_staff or user.profile.pode_acessar_clientes
        )

class UsuarioMixin(UserPassesTestMixin):
    """Usuários: REPRESENTANTE (próprio), COMERCIAL, DIRETORIA (view), ADMIN"""
    def test_func(self):
        user = self.request.user
        return user.is_authenticated and (
            user.is_staff or user.profile.pode_acessar_usuarios
        )

class MetaMixin(UserPassesTestMixin):
    """Metas: COMERCIAL, DIRETORIA (view), ADMIN"""
    def test_func(self):
        user = self.request.user
        return user.is_authenticated and (
            user.is_staff or user.profile.pode_acessar_metas
        )
...
def get_dias_uteis_corridos(mes, ano, feriados_datas):
    """
    Retorna o número de dias úteis que já foram finalizados (exclui o dia de hoje).
    """
    hoje = date.today()
    if ano > hoje.year or (ano == hoje.year and mes > hoje.month):
        return 0
...
def get_dashboard_mensal(request):
    ...
    feriados_mes = Feriado.objects.filter(data__year=ano, data__month=mes)
    feriados_datas = set(feriados_mes.values_list('data', flat=True))
    dias_rest_uteis = get_dias_uteis_restantes(mes, ano, feriados_datas)
    dias_corr_uteis = get_dias_uteis_corridos(mes, ano, feriados_datas)

    context.update({
        'mensal_faturamento': fat_total,
        'mensal_servicos_fechados': qtd_total,
        'mensal_nome_mes': calendar.month_name[mes].capitalize(),
        'mensal_dias_restantes': dias_rest_uteis,
        'mensal_dias_corridos': dias_corr_uteis,
    ...
    # Cálculo da média de faturamento diário (REALIZADA)
    fat_total_decimal = Decimal(str(fat_total))
    if dias_corr_uteis > 0:
        media_diaria = fat_total_decimal / Decimal(str(dias_corr_uteis))
    else:
        media_diaria = Decimal('0.00')
    
    # Previsão Real (Média * dias futuros + faturado acumulado)
    # dias_rest_uteis inclui hoje. Se hoje é útil, dias_futuros = dias_rest_uteis - 1
    hoje = date.today()
    hoje_is_util = (hoje.weekday() < 5 and hoje not in feriados_datas)
    dias_futuros = max(0, dias_rest_uteis - 1) if hoje_is_util else dias_rest_uteis
    previsao_real = (media_diaria * Decimal(str(dias_futuros))) + fat_total_decimal
    
    context.update({
        'mensal_media_faturamento_diario': media_diaria,
        'mensal_previsao_real': previsao_real,
    })

    if val_meta > 0:
        faltante = max(Decimal('0.00'), val_meta - fat_total_decimal)
        
        # Cálculo do faturamento diário necessário
        if dias_rest_uteis > 0:
            diario_necessario = faltante / Decimal(str(dias_rest_uteis))
        elif faltante > 0:
            diario_necessario = faltante # Se não há mais dias úteis mas falta meta
        else:
            diario_necessario = Decimal('0.00')

        context.update({
            'mensal_percentual_meta': (float(fat_total) / float(val_meta)) * 100 if val_meta > 0 else 0,
            'mensal_faturamento_faltante': faltante,
            'mensal_faturamento_diario_necessario': diario_necessario
        })
    ...
    if user.is_staff or user.profile.tem_acesso_gestao or user.profile.is_diretoria or user.profile.is_gerente_operacional:
        perf = []
...
class RepresentanteListView(LoginRequiredMixin, UsuarioMixin, ListView):
    model = User
...
class RepresentanteCreateView(LoginRequiredMixin, GestaoRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        # Valida ambos os forms
        if profile_form.is_valid():
            with transaction.atomic():
                # 1. Salva o User
                user = form.save(commit=False)

                # 2. Define a senha padrão 'mudarsenha'
                user.set_password('mudarsenha')
                user.save()

                # 3. Salva o Profile (já existe via signal, então fazemos update)
                ...
                # 4. Atualiza eh_representante baseado no setor
            ...
            # Se profile_form tiver erros, recarrega o form
    ...
    def form_valid(self, form):
        ...
        if profile_form.is_valid():
            with transaction.atomic():
                ...
                # 3. Atualiza eh_representante baseado no setor
                profile.eh_representante = (profile.setor == 'REPRESENTANTE')
                profile.save()
...
class ClienteListView(LoginRequiredMixin, ClienteMixin, ListView):
    model = Cliente
...
class ServicoListView(LoginRequiredMixin, ServicoMixin, TemplateView):
    template_name = 'app/servico_list.html'

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)

        # Flag para saber se é gerente (somente visualização)
        context['is_gerente_readonly'] = (self.request.user.profile.is_gerente_operacional or self.request.user.profile.is_diretoria)
        hoje = date.today()
        user = self.request.user
...
class ServicoDeleteView(LoginRequiredMixin, GestaoRequiredMixin, DeleteView):
    model = Servico
    # Template padrão (caso acesse pela URL direto no navegador)
    template_name = 'app/servico_confirm_delete.html'
    success_url = reverse_lazy('app:servico-list')

    def get_template_names(self):
        """
        Se a requisição for HTMX (dentro do modal), usa um template parcial
        para não carregar o site inteiro dentro da janelinha.
        """
        if self.request.htmx:
            return ['app/partials/_servico_confirm_delete_modal.html']
        return [self.template_name]

    def delete(self, request, *args, **kwargs):
        """Deleta e força refresh"""
        self.object = self.get_object()
        self.object.delete()

        # Para requisições HTMX (vinda do formulário de confirmação do modal)
        if request.htmx:
            html = '<script>window.top.location.reload(true);</script>'
...
class MetaListView(LoginRequiredMixin, MetaMixin, ListView):
    model = Meta
...
def agenda_view(request):
    ...
    if not (request.user.is_staff or request.user.profile.pode_acessar_agenda):
        return HttpResponse("Acesso Negado", status=403)
...
def prospeccao_view(request):
    ...
    # --- LÓGICA DE PERMISSÃO ---
    # Se NÃO for Gestão/Diretoria/Gerente (apenas view), vê apenas as suas
    ...
    context = {
        ...
        'is_readonly': request.user.profile.is_diretoria or request.user.profile.is_gerente_operacional,
    }
...
def criar_prospeccao(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # --- LÓGICA DE CRIAÇÃO E ATRIBUIÇÃO ---
            prospeccao.criado_por = request.user
            
            # Se não for selecionado um responsável, o criador assume
            if not prospeccao.atribuido_a:
                prospeccao.atribuido_a = request.user
...
def detalhe_prospeccao(request, pk):
    ...
    # Permite acesso para: ADMIN, COMERCIAL, DIRETORIA, GERENTE, ou criador
    if not (request.user.is_staff or
            request.user.profile.tem_acesso_gestao or
            request.user.profile.is_diretoria or
            request.user.profile.is_gerente_operacional or
            prospeccao.criado_por == request.user or
            prospeccao.atribuido_a == request.user):
        return HttpResponse("Acesso Negado", status=403)
...
def editar_prospeccao(request, pk):
    ...
    if request.method == 'POST':
        form = ProspeccaoEditForm(request.POST, instance=prospeccao, user=request.user)
        if form.is_valid():

            changes = []
            
            # A lógica de delegação agora é via campo 'atribuido_a' no formulário
            # O ProspeccaoEditForm já inclui 'atribuido_a' nos campos editados
            
            if form.has_changed():
                for field_name in form.changed_data:
                    # Skip delegado_para as it is handled manually
                    if field_name == 'delegado_para':
                        continue
        ...
        form = ProspeccaoEditForm(instance=prospeccao, user=request.user)
...
def dashboard_prospeccao(request):
    ...
    if not request.user.is_staff and not request.user.profile.tem_acesso_gestao:
        ...
        qs = qs.filter(atribuido_a_id=rep_id)
    ...
    if request.user.is_staff or request.user.profile.tem_acesso_gestao:
        perf_data = qs.filter(status='FECHADO').values('atribuido_a__username') \
                      .annotate(total=Sum('valor_total')).order_by('-total')

        labels_perf = [item['atribuido_a__username'] for item in perf_data]
...
def tarefas_view(request):
    """
    View principal da agenda de tarefas com calendário e timeline.
    Usando model TarefaAgendada.
    """
    try:
        data_atual_str = request.GET.get('dia', date.today().isoformat())
        data_selecionada = datetime.strptime(data_atual_str, '%Y-%m-%d').date()
    except ValueError:
        data_selecionada = date.today()

    try:
        mes = int(request.GET.get('mes', data_selecionada.month))
        ano = int(request.GET.get('ano', data_selecionada.year))
    except (ValueError, TypeError):
        mes = data_selecionada.month
        ano = data_selecionada.year

    # ATUALIZAÇÃO AUTOMÁTICA DE TAREFAS ATRASADAS
    # Define como NÃO REALIZADA qualquer tarefa pendente anterior a hoje
    TarefaAgendada.objects.filter(
        status='PENDENTE',
        data_agendamento__lt=date.today()
    ...
    user = request.user
    q_user = Q(criado_por=user) | Q(atribuido_a=user)
    tarefas_qs = TarefaAgendada.objects.filter(q_user)

    # Preenchimento inicial
    primeiro_dia_mes = date(ano, mes, 1)
    dia_semana_primeiro = (primeiro_dia_mes.weekday() + 1) % 7 
    dias_vazios_inicio = range(dia_semana_primeiro)

    # Identificar quais dias tem tarefa e quais status
    dias_status_map = defaultdict(set)
    tarefas_mes = tarefas_qs.filter(
        data_agendamento__year=ano, 
        data_agendamento__month=mes
    )
    for t in tarefas_mes:
        dias_status_map[t.data_agendamento].add(t.status)

    # Preparar estrutura para o calendário (Data + Statuses)
    dias_do_mes = []
    for dia in cal.itermonthdates(ano, mes):
        if dia.month == mes:
            dias_do_mes.append({
                'date': dia,
                'statuses': dias_status_map.get(dia, set())
            })

    # Tarefas do dia selecionado
    tarefas_do_dia = tarefas_qs.filter(data_agendamento=data_selecionada).order_by('data_agendamento', 'data_criacao')
    
    data_base_mes = date(ano, mes, 1)
    prox_mes_data = (data_base_mes + timedelta(days=32)).replace(day=1)
    ant_mes_data = (data_base_mes - timedelta(days=1)).replace(day=1)
    ...
    context = {
        ...
        'ano_atual': ano,
        'dias_do_mes': dias_do_mes, # Lista de dicts: {'date': date, 'statuses': {set}}
        'dias_vazios_inicio': dias_vazios_inicio,
        'dia_selecionado_str': data_selecionada.isoformat(),
        'dia_selecionado_formatado': data_selecionada.strftime('%d/%m/%Y'),
        'tarefas_do_dia': tarefas_do_dia,
        'mes_anterior': ant_mes_data.month,
        'ano_anterior': ant_mes_data.year,
        'mes_proximo': prox_mes_data.month,
        'ano_proximo': prox_mes_data.year,
    }
    return render(request, 'app/tarefas.html', context)

@login_required
def criar_tarefa_agendada(request):
    """
    Cria uma nova TarefaAgendada.
    """
    if request.method == 'POST':
        form = TarefaAgendadaForm(request.POST, user=request.user)
        if form.is_valid():
            tarefa = form.save(commit=False)
            tarefa.criado_por = request.user
            # Se attributed_a não vier (campo oculto), atribui a si
            if not form.cleaned_data.get('atribuido_a'):
                tarefa.atribuido_a = request.user
            tarefa.save()
            return HttpResponse(status=204) # HTMX success
    else:
        form = TarefaAgendadaForm(user=request.user)
    
    # Usaremos um novo template ou o mesmo genérico mudando a URL de post
    context = {
        'form': form,
        'post_url': reverse('app:criar-tarefa-agendada'), 
        'modal_title': 'Adicionar Nova Tarefa'
    }
    return render(request, 'app/partials/_tarefa_agendada_form_modal.html', context)

@login_required
def marcar_visualizada(request, pk):
    tarefa = get_object_or_404(TarefaAgendada, pk=pk)
    if tarefa.atribuido_a == request.user and not tarefa.visualizada_em:
        tarefa.visualizada_em = timezone.now()
        tarefa.save()
    return HttpResponse(status=200)

@login_required
def check_tarefas_pendentes(request):
    novas_tarefas = TarefaAgendada.objects.filter(
        atribuido_a=request.user, 
        visualizada_em__isnull=True
    )
    
    if not novas_tarefas.exists():
        return HttpResponse("") 
        
    count = novas_tarefas.count()
    
    html = f"""
    <div class="modal fade" id="alert-tarefas-modal" tabindex="-1" aria-hidden="true">
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header bg-warning">
            <h5 class="modal-title text-dark"><i class="bi bi-exclamation-triangle"></i> Novas Tarefas Atribuídas</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <div class="modal-body">
            <p>Você tem <strong>{count}</strong> nova(s) tarefa(s) atribuída(s) que precisa(m) de sua atenção.</p>
            <ul class="list-group">
                {''.join([f'<li class="list-group-item">{t.titulo} <small class="text-muted">({t.data_agendamento})</small></li>' for t in novas_tarefas[:5]])}
                {f'<li class="list-group-item text-center text-muted">... e mais {count-5}</li>' if count > 5 else ''}
            </ul>
          </div>
          <div class="modal-footer">
            <button type="button" class="btn btn-primary" onclick="marcarTarefasLidas()" data-bs-dismiss="modal">Entendi</button>
          </div>
        </div>
      </div>
    </div>
    <script>
        var alertModal = new bootstrap.Modal(document.getElementById('alert-tarefas-modal'));
        alertModal.show();
        function marcarTarefasLidas() {{
            fetch('{reverse("app:marcar_todas_visualizadas")}', {{
                method: 'POST',
...
@login_required
def marcar_todas_visualizadas(request):
    if request.method == 'POST':
        TarefaAgendada.objects.filter(atribuido_a=request.user, visualizada_em__isnull=True).update(visualizada_em=timezone.now())
        return HttpResponse("OK")
    return HttpResponse(status=400)

@login_required
def concluir_tarefa(request, pk, status):
    tarefa = get_object_or_404(TarefaAgendada, pk=pk)
    if status not in ['REALIZADA', 'NAO_REALIZADA']:
...
@login_required
def detalhe_tarefa_agendada(request, pk):
    tarefa = get_object_or_404(TarefaAgendada, pk=pk)
    return render(request, 'app/partials/_detalhe_tarefa_agendada_modal.html', {'tarefa': tarefa})


# --- GESTÃO DE FERIADOS (AGENDA DE DIAS ÚTEIS) ---
...
@login_required
def gerenciar_feriados_modal(request):
    """
    Exibe modal para listar e gerenciar feriados de um determinado mês/ano.
    """
    if not (request.user.is_staff or request.user.profile.tem_acesso_gestao):
        return HttpResponse("Acesso negado.", status=403)

    mes = clean_int(request.GET.get('mes'), date.today().month)
    ano = clean_int(request.GET.get('ano'), date.today().year)
    
    feriados = Feriado.objects.filter(data__year=ano, data__month=mes).order_by('data')
    form = FeriadoForm(initial={'data': date(ano, mes, 1)})
    
    return render(request, 'app/partials/_modal_feriados.html', {
        'feriados': feriados,
        'form': form,
        'mes_selecionado': mes,
        'ano_selecionado': ano,
...
def adicionar_feriado(request):
    """
    Adiciona um feriado via HTMX e atualiza a lista no modal.
    """
    if not (request.user.is_staff or request.user.profile.tem_acesso_gestao):
        return HttpResponse("Acesso negado.", status=403)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Recarrega a lista para o HTMX
            feriados = Feriado.objects.filter(data__year=ano, data__month=mes).order_by('data')
            # Trigger para o dashboard atualizar o balão de dias úteis
            context = {
                'feriados': feriados,
                'form': FeriadoForm(initial={'data': date(ano, mes, 1)}),
                'mes_selecionado': mes,
...
@login_required
def deletar_feriado(request, pk):
    """
    Deleta um feriado via HTMX e atualiza a lista no modal.
    """
    if not (request.user.is_staff or request.user.profile.tem_acesso_gestao):
        return HttpResponse("Acesso negado.", status=403)
