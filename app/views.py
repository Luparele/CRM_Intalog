...
def home_page(request):
    ...
    # 1. Define o template com base no perfil
    if profile.is_representante:
        template_name = 'app/dashboard_representante.html'
        context = {}
    else:
        template_name = 'app/dashboard_admin.html'
        context = {}
    
    # 2. Busca o último lançamento
    base_qs = Servico.objects.all()
    if profile.is_representante:
        base_qs = base_qs.filter(cliente__cadastrado_por=user)
        
    ultimo_lancamento = base_qs.order_by('-data_registro').first()
    context['ultimo_lancamento'] = ultimo_lancamento
    
    # 3. CRÍTICO: Injeta o contexto dos filtros
...
class ClienteCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        self.object = form.save(commit=False)
        if not self.object.cadastrado_por:
            self.object.cadastrado_por = self.request.user
        self.object.save()
...
class ServicoCreateView(LoginRequiredMixin, GestaoRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        self.object = form.save(commit=False)
        self.object.fechado_por = self.request.user 
        self.object.save()
        return super().form_valid(form)
...
class ServicoUpdateView(LoginRequiredMixin, GestaoRequiredMixin, UpdateView):
    ...
    def get_success_url(self):
        # Se for modal, o sucesso é redirecionar para o historico do cliente
        if self.request.htmx:
             return reverse('app:servico-historico-modal', kwargs={
                'pk': self.object.cliente.id, 
                'mes': self.object.data_servico.month, 
                'ano': self.object.data_servico.year
            })
    ...
    def form_valid(self, form):
        ...
        if self.request.htmx:
            response['HX-Trigger'] = 'reload-dashboard' 
        return response
...
def prospeccao_view(request):
    """ Exibe o Funil de Vendas (Kanban) """
    ...
    qs = Prospeccao.objects.all().select_related('cliente', 'criado_por')
    ...
    if data_ini: qs = qs.filter(data_criacao__date__gte=data_ini)
    if data_fim: qs = qs.filter(data_criacao__date__lte=data_fim)

    context = {
        'novas': qs.filter(status='NOVA').order_by('-data_criacao'),
        'negociando': qs.filter(status='NEGOCIANDO').order_by('-data_criacao'),
        'finalizadas_fechado': qs.filter(status='FECHADO').order_by('-data_finalizacao')[:5],
        'finalizadas_desistencia': qs.filter(status='DESISTENCIA').order_by('-data_finalizacao')[:5],
        'finalizadas_perdida': qs.filter(status='PERDIDA').order_by('-data_finalizacao')[:5],
        'representantes': User.objects.filter(is_active=True).order_by('username'),
    }
...
def criar_cliente_prospeccao_modal(request):
    """ Modal para criar um ClienteProspect rápido (usado dentro do form de Prospecção) """
...
def salvar_cliente_prospeccao(request):
    """ Salva o prospect e retorna o select atualizado ou o form com erro """
    form = ClienteProspectForm(request.POST)
    if form.is_valid():
        prospect = form.save(commit=False)
        prospect.cadastrado_por = request.user
        prospect.save()
        
        # Retorna o <select> do form principal atualizado, com o novo prospect selecionado
        # Precisamos instanciar o form principal para renderizar o campo 'cliente'
        main_form = ProspeccaoForm(user=request.user)
        # Hack para setar o valor inicial do campo recém criado
        main_form.fields['cliente'].initial = prospect.id
        
        # Renderiza apenas o widget do campo cliente
        return HttpResponse(main_form['cliente'].as_widget())
...
def editar_prospeccao(request, pk):
    ...
    return render(request, 'app/partials/_prospeccao_edit_form_modal.html', {
        'form': form, 'prospeccao': prospeccao
    })
...
def consulta_cnpj_api(request, cnpj):
    """
    Consulta CNPJ na BrasilAPI e retorna JSON.
    Tenta até 40 vezes em caso de falha de conexão ou Rate Limit (429).
    """
    # Remove caracteres não numéricos
    cnpj_limpo = ''.join(filter(str.isdigit, cnpj))
    
    if len(cnpj_limpo) != 14:
        return JsonResponse({'error': 'CNPJ inválido (deve conter 14 dígitos).'}, status=400)
    ...
    for tentativa in range(1, max_tentativas + 1):
        try:
            # Timeout curto para não travar muito tempo em cada tentativa
            ...
            if response.status_code == 200:
                ...
                endereco_completo = f"{logradouro}, {numero} - {bairro}, {municipio} - {uf}"
                if complemento:
                ...
                # Se não existe, não adianta tentar de novo
                return JsonResponse({'error': 'CNPJ não encontrado na base de dados.'}, status=404)
                ...
                # Outros erros (500, etc), tenta de novo
                time.sleep(1)
                continue
    ...
    # Se saiu do loop, falhou todas as vezes
    return JsonResponse({'error': 'Não foi possível obter dados. O sistema tentou conectar 40 vezes sem sucesso.'}, status=500)
